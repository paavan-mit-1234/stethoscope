...
pub fn any_to_json(v: &AnyValue) -> Json {
    match &v.value {
        ...
        Some(Value::DoubleValue(d)) => Number::from_f64(*d).map(Json::Number).unwrap_or(Json::Null),
        Some(Value::BytesValue(b)) => Json::String(hex::encode(b)),
        Some(Value::ArrayValue(a)) => Json::Array(a.values.iter().map(any_to_json).collect()),
...
pub fn get_str<'a>(attrs: &'a [KeyValue], key: &str) -> Option<&'a str> {
    attrs
        .iter()
        .find(|k| k.key == key)
        .and_then(|k| match &k.value.as_ref()?.value {
